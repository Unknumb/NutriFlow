...
export class PreparacionesController {
  ...
  @ApiResponse({ status: 400, description: 'Datos inválidos o alimentos inexistentes.' })
  create(@Body() createPreparacionDto: CreatePreparacionDto, @CurrentUser() user: any) {
    return this.preparacionesService.create(createPreparacionDto, user.userId);
  }

  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar una preparación propia (las del sistema son solo lectura)' })
  @ApiResponse({ status: 403, description: 'Preparación del sistema, no editable.' })
  ...
  update(
    ...
    return this.preparacionesService.update(id, updatePreparacionDto, user.userId);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Eliminar una preparación propia (las del sistema son solo lectura)' })
  @ApiResponse({ status: 403, description: 'Preparación del sistema, no eliminable.' })
  @ApiResponse({ status: 404, description: 'Preparación no encontrada.' })
