...
export interface UpdatePlanificacionData {
    nombre?: string;
    calorias_totales?: number;
    distribucion_macros?: Record<string, number>;
}
...
export const planificacionesApi = {
    ...
    updatePlanificacion: async (id: string, data: UpdatePlanificacionData): Promise<Planificacion> => {
        const response = await apiClient.patch(`/planificaciones/${id}`, data);
        return response.data;
    },
