...
export const DraggableGroupHeader = ({ group }: Props) => {
    ...
    return (
        <th 
            ...
            {...listeners}
            {...attributes}
            className={`${group.headerBg} text-white border border-gray-200 text-center px-2 py-2 w-20 cursor-grab active:cursor-grabbing hover:opacity-90 transition-opacity ${isDragging ? 'opacity-50 ring-2 ring-white ring-inset' : ''}`}
