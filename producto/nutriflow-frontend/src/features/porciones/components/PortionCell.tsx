  ...
  if (value === 0) {
    return (
      <button
        type="button"
        onClick={onIncrement}
        className="w-6 h-6 mx-auto rounded-full border border-dashed border-mist text-ink-soft/60 hover:border-pine-soft hover:text-pine-soft flex items-center justify-center transition-colors relative after:absolute after:-inset-2.5 after:content-[''] active:scale-95"
  ...
  return (
    <>
      ...
      <div
        ...
        {...listeners}
        {...attributes}
        className={`${cellBg} rounded-lg px-2 py-0.5 mx-auto w-14 xl:w-12 flex flex-col items-center shadow-sm cursor-grab active:cursor-grabbing hover:ring-2 hover:ring-offset-1 hover:ring-mist transition-all touch-none ${isDragging ? "opacity-50 scale-105" : ""} overflow-hidden`}
        title="Arrastra hacia arriba para restar"
      >
        <button
          type="button"
          onClick={onIncrement}
          className={`text-xs py-1.5 xl:text-[10px] xl:py-0 ${textBtn} opacity-60 hover:opacity-100 w-full font-bold active:bg-white/60 transition-colors flex items-center justify-center`}
