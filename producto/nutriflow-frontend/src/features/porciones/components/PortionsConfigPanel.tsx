  ...
  return (
    <>
      <button
        ...
        className="w-full sm:w-auto justify-center bg-white border border-mist text-ink-soft hover:bg-porcelain px-4 py-2 sm:py-1.5 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all shadow-sm shrink-0 whitespace-nowrap"
      >
      ...
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-100 bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="flex items-center justify-between p-5 border-b border-mist/70 bg-porcelain/50">
          <div>
            ...
            <p className="text-sm text-ink-soft">
              Ajusta los grupos y tiempos de comida.
            </p>
          </div>
          <button
            onClick={() => setIsOpen(false)}
            ...
            <div className="space-y-2">
              {comidas.map((meal) => {
                ...
                return (
                  <div
                    key={meal.id}
                    className="flex items-center gap-2 p-2.5 rounded-xl border border-mist/70"
                  >
                    <input
                      type="checkbox"
