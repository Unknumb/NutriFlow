      ...
      <div className="p-8 max-w-4xl mx-auto">
        ...
        <div className="overflow-hidden rounded-xl border border-gray-300 shadow-md">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="bg-teal-700 text-white text-sm font-bold px-4 py-3 text-center border-r border-teal-600 w-24">
                  HORA
                </th>
                <th className="bg-teal-700 text-white text-sm font-bold px-4 py-3 text-center border-r border-teal-600 w-32">
                  COMIDA
                </th>
                <th className="bg-teal-700 text-white text-sm font-bold px-4 py-3 text-center border-r border-teal-600 w-36">
                  PORCIÓN
                </th>
                <th className="bg-teal-700 text-white text-sm font-bold px-4 py-3 text-center border-r border-teal-600 ">
                  OPCIONES
                </th>
              </tr>
            </thead>
                  ...
                  <React.Fragment key={meal.id}>
                    {activeGroups.length === 0 ? (
                      <tr className={rowBg}>
                        ...
                        <td className="border border-gray-200 px-4 py-3 text-center align-middle font-bold text-gray-900">
                          {meal.name}
                        </td>
                        <td
                          colSpan={2}
                          className="border border-gray-200 px-4 py-3 text-center text-gray-400 italic"
                        >
                      ...
                      activeGroups.map((group, groupIdx) => (
                        <tr key={`${meal.id}-${group.id}`} className={rowBg}>
                          {groupIdx === 0 && (
                            <>
                              <td
                                className="border border-gray-200 px-4 py-3 text-center align-top font-mono text-sm text-gray-700 font-medium"
                                rowSpan={rowCount}
                              >
                                {meal.time.split("\n").map((t, i) => (
                                  <div key={i}>{t}</div>
                              ...
                              <td
                                className="border border-gray-200 px-4 py-3 text-center align-middle font-bold text-gray-900"
                                rowSpan={rowCount}
                              >
                                {meal.name}
                              </td>
                          ...
                          <td className="border border-gray-200 px-4 py-2.5 align-middle">
                            <div className="flex items-center gap-2">
                              <span
                                className={`inline-flex items-center justify-center w-7 h-7 rounded-full ${group.bg} ${group.text} font-bold text-sm shrink-0`}
                              >
                          ...
                          <td className="border border-gray-200 px-4 py-2.5 align-middle text-sm text-gray-600">
                            {group.options}
                          </td>
                        </tr>
                      ))
        ...
        <div className="mt-4 bg-gray-50 border border-gray-200 rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {ALL_GROUPS.filter(g => targets[g.id] > 0 || getGroupTotal(g.id) > 0).map((g) => {
