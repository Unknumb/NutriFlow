...
export const Sidebar = ({ isOpenMobile = false, onClose }: SidebarProps) => {
    ...
    return (
        <aside className={`bg-pine text-porcelain flex flex-col h-screen transition-all duration-300 ease-in-out overflow-hidden fixed inset-y-0 left-0 z-50 ${isOpenMobile ? 'translate-x-0 shadow-2xl' : '-translate-x-full'} md:relative md:sticky md:top-0 md:translate-x-0 md:shadow-none ${isCollapsed ? 'w-20' : 'w-64'}`}>
            {/* ── Header ── */}
            {isCollapsed ? (
                /* Colapsado: solo el botón Menu para expandir */
                ...
                /* Expandido: logo + botón X para colapsar */
            ...
            {/* ── Navegación ── */}
            <nav className="flex-1 px-3 pb-4 overflow-y-auto space-y-6" onClick={onClose}>
            ...
            {/* Modal de confirmación de cierre de sesión (portal para evitar bugs de z-index) */}
            {showLogoutModal &&
